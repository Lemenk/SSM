 ...
 * @Author: Lemenk
 * @Blog: www.lemenk.top
 * @Date: Created in 2020/2/11 20:34
...
public class JdbcTemplateDemo3 {
    ...
    public static void main(String[] args) {
        ...
        //3.执行操作
            //保存
        //jt.update("insert into account(name,money)value (?,?)","fff",3333f);
            //更新
        //jt.update("update account set name=?,money=? where id=?","test",4567,4);
            //删除
        //jt.update("delete from account where id=?",5);
            //查询所有
        //List<Account> accounts = jt.query("select * from account where money > ?",new AccountRowMapper(),1000f);//其中AccountRowMapper需要自己定义
